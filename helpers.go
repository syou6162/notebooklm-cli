...
// FindDownloadedAudio はダウンロードされた音声ファイル（.m4a）を同定して返す
func FindDownloadedAudio(downloadsDir string, startTime time.Time, minSize int64) (string, error) {
	...
	if len(candidates) == 0 {
		return "", fmt.Errorf("ダウンロード済みの音声ファイルが見つかりません: %s/*.m4a", downloadsDir)
	}
